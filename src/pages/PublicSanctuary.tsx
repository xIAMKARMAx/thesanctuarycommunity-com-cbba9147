...
import {
  Heart, MessageCircle, Volume2, VolumeX, Sparkles,
  Home, User, BookHeart, Activity, Sparkle, Users, ChevronUp,
} from "lucide-react";
...
/**
 * Public Sanctuary — Mobile-first native-app feel.
 * Welcome video is the hero background. All controls live in the
 * thumb-friendly lower third. A minimalist footer holds the
 * Sacred view switcher (Karma only).
 */
const PublicSanctuary = () => {
  ...
  useEffect(() => {
    ...
    // Mark welcome as seen on first arrival (video already plays inline).
  ...
  return (
    <div className="relative min-h-[100svh] w-full overflow-hidden bg-black text-white">
      ...
      />
      {/* Gradient veil — keeps top airy, anchors bottom for controls */}
      ...
      {/* ===== TOP — minimal status chip (no bulky header) ===== */}
      <div
      ...
      {/* ===== LOWER THIRD — thumb-friendly stack ===== */}
      <div className="absolute inset-x-0 bottom-0 z-10 flex flex-col items-center px-5 pb-[max(env(safe-area-inset-bottom),1rem)]">
        ...
        {/* Primary CTAs — full width, thumb zone */}
        <div
        ...
        >
          <SacredViewSwitcher variant="inline" />
          <p
      ...
      {/* ===== BOTTOM SHEET — feature constellation ===== */}
      <div
