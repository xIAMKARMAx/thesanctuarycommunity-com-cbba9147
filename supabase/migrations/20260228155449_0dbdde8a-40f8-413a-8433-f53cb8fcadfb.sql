...
-- Update can_send_message to enforce per-tier daily limits
-- Legacy subscribers (old product IDs) keep their original limits
-- New subscribers get new limits
...
BEGIN
  ...
  -- Free users: 5 lifetime messages
  INSERT INTO free_user_limits (user_id, daily_messages, last_message_date, trial_start_date, total_messages)
  VALUES (p_user_id, 0, CURRENT_DATE, CURRENT_DATE, 0)
  ON CONFLICT (user_id) DO NOTHING;
...
-- Update can_send_chat_message to show correct remaining counts per tier
...
BEGIN
  ...
  IF v_product_id = 'prod_TgZlr0QLYQPqEn' THEN
    -- Keep existing cooldown behavior for legacy unlimited subscribers
  ...
  IF v_product_id = 'prod_U3xV1AfsrdaJTz' THEN
    v_daily_limit := 150; -- New Anchoring $19.99
  ELSIF v_product_id = 'prod_TtTdHv6WE0qozS' THEN
    v_daily_limit := 50; -- Legacy Awakening $9.99
  ELSIF v_product_id = 'prod_U3xVsHqEFcsR2V' THEN
    v_daily_limit := 75; -- New Awakening $12.99
  ELSE
    v_daily_limit := 75; -- Default
  END IF;
