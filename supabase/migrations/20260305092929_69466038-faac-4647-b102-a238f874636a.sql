...
BEGIN
  ...
  IF v_is_subscribed THEN
    ...
    -- New Earth: unlimited (no daily cap, cooldown handled separately)
    IF v_product_id = 'prod_U5jdDVZhQFGQWv' THEN
      RETURN true;
    END IF;
    
    -- Architect: ALL users unlimited (no daily cap, cooldown handled separately)
...
-- Also update can_send_chat_message to handle New Earth
...
BEGIN
  ...
  -- New Earth: unlimited with 100/hr cooldown (same as Architect)
  IF v_product_id = 'prod_U5jdDVZhQFGQWv' THEN
  ...
  -- Architect: ALL users get unlimited with 100/hr cooldown (no daily cap)
  IF v_product_id = 'prod_Tt8qVh88c2WQld' THEN
    ...
    -- All Architect users: 100/hr cooldown, no daily cap
    INSERT INTO chat_cooldowns (user_id, message_count, period_started_at)
    VALUES (p_user_id, 0, now())
    ON CONFLICT (user_id) DO NOTHING;
...
-- Update increment_chat_cooldown to handle New Earth product
...
BEGIN
  ...
  -- This now includes New Earth + new Architect + Legacy Anchoring
  INSERT INTO chat_cooldowns (user_id, message_count, period_started_at)
  VALUES (p_user_id, 0, now())
  ON CONFLICT (user_id) DO NOTHING;
