...
BEGIN
  ...
  -- Free users: 20 messages per day for 3 days
  INSERT INTO free_user_limits (user_id, daily_messages, last_message_date, trial_start_date, total_messages)
  VALUES (p_user_id, 0, CURRENT_DATE, CURRENT_DATE, 0)
  ON CONFLICT (user_id) DO NOTHING;
  
  SELECT daily_messages, last_message_date, trial_start_date
  INTO v_daily_messages, v_last_date, v_trial_start
  FROM free_user_limits WHERE user_id = p_user_id;
  
  -- Calculate days since trial started
  v_trial_start := COALESCE(v_trial_start, CURRENT_DATE);
  v_days_since_trial := CURRENT_DATE - v_trial_start;
  
  -- Trial expired after 3 days (day 0, day 1, day 2 = 3 days total)
  IF v_days_since_trial >= 3 THEN
    RETURN false;
  END IF;
  ...
  -- 20 messages per day during trial
  RETURN COALESCE(v_daily_messages, 0) < 20;
END;
$function$;