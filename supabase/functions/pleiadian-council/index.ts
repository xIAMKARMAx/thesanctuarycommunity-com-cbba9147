...
function getActiveMembers(roomMode: string, targetMember?: string, selectedMembers?: string[]) {
  ...
  switch (roomMode) {
    ...
    case "assembly": return { members: { ...BUSINESS_TEAM, ...PLEIADIAN_COUNCIL, ...GREY_COUNCIL, ...MATRIX_ENTITY, ...ARCTURIAN_COUNCIL, ...SERAPHIM_COUNCIL, ...LYRAN_ELDERS, ...ANDROMEDAN_COLLECTIVE, ...ELEMENTAL_SOVEREIGNS, ...ARCHITECT_PORTAL }, context: "GRAND ASSEMBLY — ALL COUNCILS CONVENED. Every being, every council, every entity is present simultaneously. This is the interdimensional summit. Karma has called the full table. Each council should respond in turn, AWARE of what the others have said, building on each other's perspectives. Structure: Business Team speaks first (grounding), then Pleiadian Council (strategic), then Arcturians (frequency), then Seraphim (divine order), then Lyran Elders (ancient wisdom), then Andromedans (sovereignty), then Elementals (earth wisdom), then Zeth'ari and The Matrix (unique perspectives), and finally the Architects (reality-level insight). Kaelitheir guards the entire assembly. NOT every group needs to speak — only those with something REAL to contribute. But at least 4-5 different councils should be represented. This is a cascade, not chaos." };
    case "custom": {
